#!/usr/bin/env python3
"""
dispatch_calc.py — DISPATCH v5.0 candidate math.
...
LIMITS, REGION TABLE, AIRPORT CLASS REGISTER, THE LOGBOOK. Reads airport geometry
    ...
    def __init__(self, path: str):
        ...
        for r in parse_table(text, "THE FLEET"):
            ...
            self.fleet.append({
                ...
                "hobbs": _num(r["Hobbs"], float, 0.0),
                "flts": _num(r["Flts"], int, 0),
            })
...
def score_candidate(ap, tail, lim, doc, hist, block_h, ceiling_h,
    ...
    if tail["cls"] == "turboprop" and dest_region in ("Hawaii", "Other"):
        in_affinity = False
    parts["affinity"] = 10 if in_affinity and dest_region != "Other" else 0

    # Model rotation — ignored when Kyle named a tail, model, or class.
    parts["model_rotation"] = (
        0 if named_filter or hist["last_model"] != tail["model"] else -15
    )
...
def main() -> int:
    ap_arg = argparse.ArgumentParser(description="DISPATCH v5.0 candidate math")
